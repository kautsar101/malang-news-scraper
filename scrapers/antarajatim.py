...
BASE_URL = "https://jatim.antaranews.com/kabar-jatim/malang-raya"
SOURCE = "antara_jatim"
OUTPUT_PATH = "csv/antara_jatim_articles.csv"
...
def scrape_list(cutoff, max_pages=200):
    ...
    for page_num in range(1, max_pages + 1):
        ...
        print(f"Scraping Antara Jatim page {page_num}: {url}", flush=True)

        try:
            soup = fetch_html(url)
        except Exception as error:
            print(f"Gagal buka Antara Jatim page {page_num}: {error}", flush=True)
            break

        cards = soup.select(".card__post")
...
def extract_content(soup):
    content = (
        ...
        or soup.select_one(".wrap__article-detail-content")
...
def scrape(max_pages=200):
    ...
    for index, row in urls_df.iterrows():
        try:
            ...
            print(f"[{index + 1}/{len(urls_df)}] Antara Jatim failed: {error}", flush=True)
            continue
        ...
        if not article.get("content"):
            print(f"Antara Jatim content kosong, skip: {row['url']}", flush=True)
            continue
    ...
    print(f"\nSaved {len(df)} Antara Jatim articles", flush=True)
    return df
